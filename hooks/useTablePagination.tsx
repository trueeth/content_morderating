...
import { setPagination, setPaginationIndex } from '@/store/reducers/pagination.reducers'
import { IReduxState } from '@/store/index'
import { IAppSlice } from '@/store/reducers'

const useTablePagination = () => {

  const [vState, setState]=useState({pageIndex:1, pageSize:10, pageTotal:5})
  ...
  useEffect(() => {
    setState({
      ...
      pageTotal: Math.ceil(appState.pagination.totalCount/appState.pagination.pageSize),
    })
  }, [appState])
  ...
  const handleChangePagination = (event: React.ChangeEvent, pageIndex: number) => {
    setState({...vState,
      pageIndex: pageIndex})
    dispatch(setPaginationIndex({
      pageIndex:pageIndex}))
